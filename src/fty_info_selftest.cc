/*  =========================================================================
    fty_info_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running fty-info selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_INFO_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_info_private_selftest (verbose, item->subtest);
#endif // FTY_INFO_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_info_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-info test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_INFO_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_info_private_selftest (verbose, test->subtest);
#endif // FTY_INFO_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
